...
/*                else if (_event.get(IUmpire.Event.NEW_TIEBREAK) != null && _event.get(IUmpire.Event.NEW_TIEBREAK) == true)
    ...
    private String _spellpoints_msg()
    {
        if (_is_tie)
        {
            ...
            {
                return String.format(myApp.get_instance().getString(R.string.tiebreak_equal), Helper.spell_the_digit(_umpire.get_players()[0].get_points(), R.array.po_skolki));
            }
            else
            {
                ...
                return String.format(myApp.get_instance().getString(R.string.tiebreak_leader), Helper.spell_the_digit(_umpire.get_serving_player().get_points(), R.array.skolko), Helper.spell_the_digit(_umpire.get_returning_player().get_points(), R.array.skolko), _winner_name);
            }
        ...
        {
            // по пятнадцати:
            if (_umpire.get_serving_player().get_points() == 1  )
            {
                return Helper.spell_the_digit(7, R.array.spell_points);
            }
            if (_umpire.get_serving_player().get_points() == 2  )
            {   // по тридцати:
                return Helper.spell_the_digit(8, R.array.spell_points);
            }
